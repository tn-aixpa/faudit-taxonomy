...
istat = pd.read_csv('data/Codici-statistici-e-denominazioni-al-01_01_2025.csv', sep=';', encoding='Windows-1252')
base = pd.read_excel('data/tassonomia_comuni.xlsx').rename(columns={'Azione':'azione'})
piani_comunali = pd.read_csv('data/azioni_complete.csv', sep=';').drop(columns='Unnamed: 0')
piani_comunali['azione'] = piani_comunali['azione'].apply(lambda x: x.replace('\xa0', ''))
piani_comunali['descrizione'] = piani_comunali['descrizione'].apply(lambda x: str(x).replace('\r\n', ' '))
...
Azione_2 = []
Levenshtein = []
for c in piani_comunali.comune_breve.unique():
