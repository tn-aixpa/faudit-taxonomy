...
def init(context):
    azioni_frequenza = context.project.get_dataitem("tassonomia_aziende_azioni_frequenza").as_df()
    azioni_unire = context.project.get_dataitem("tassonomia_aziende_azioni_unire").as_df()
...
def serve(context, event):
    ...
    if "to_merge" in path:
        summary = context.azioni_unire.rename(columns={'ID_tassonomia_1': 'id1', 'ID_tassonomia_2': 'id2', 'descrizione_1_list': 'actions1', 'descrizione_2_list': 'actions2', 'Percentuale_di_similarità': 'similarity'})[['id1', 'id2', 'actions1', 'actions2', 'similarity']]
